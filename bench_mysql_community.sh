...
COMMUNITY_ROOT=/home/shuchen1/workspace/mysql/installation/mysql_community_8.4.8
MYSQLD=$COMMUNITY_ROOT/usr/sbin/mysqld
MYSQL=$COMMUNITY_ROOT/usr/bin/mysql
MYSQLADMIN=$COMMUNITY_ROOT/usr/bin/mysqladmin
BASEDIR=$COMMUNITY_ROOT/usr
DATADIR=/home/shuchen1/workspace/mysql/data_community
SOCKET=/tmp/mysql_community.sock
PORT=3310
HAMMERDB=/home/shuchen1/workspace/mysql/HammerDB-5.0
LOG_PREFIX=/tmp/hammerdb_community

# Make libmysqlclient.so.24 and private runtime libs available
export LD_LIBRARY_PATH=$COMMUNITY_ROOT/usr/lib/x86_64-linux-gnu:$COMMUNITY_ROOT/usr/lib/mysql/private${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}
...
echo "=== MySQL Community 8.4.8 HammerDB Benchmark ==="
echo "  COMMUNITY_ROOT: $COMMUNITY_ROOT"
...
if [ -S "$SOCKET" ]; then
    $MYSQLADMIN -u root --socket=$SOCKET shutdown 2>/dev/null || true
    sleep 2
fi

taskset -c $CPUS $MYSQLD \
    --no-defaults \
    --basedir=$BASEDIR \
    --datadir=$DATADIR \
    --socket=$SOCKET \
...
for i in $(seq 1 30); do
    if $MYSQLADMIN -u root --socket=$SOCKET ping 2>/dev/null | grep -q alive; then
...
cat > /tmp/hammerdb_community_build.tcl << EOF
dbset db mysql
...
cd $HAMMERDB
./hammerdbcli auto /tmp/hammerdb_community_build.tcl
...
cat > /tmp/hammerdb_community_run.tcl << 'TCLEOF'
dbset db mysql
diset connection mysql_host 127.0.0.1
...
jobs $jobid timing
TCLEOF
sed -i "s/HAMMERDB_PORT/$PORT/g; s|HAMMERDB_SOCKET|$SOCKET|g; s/HAMMERDB_RAMPUP/$RAMPUP/g; s/HAMMERDB_DURATION/$DURATION/g; s/HAMMERDB_VU_TEST/$VU_TEST/g" /tmp/hammerdb_community_run.tcl

for RUN in $(seq 1 "$RUNS"); do
    HAMMERDB_LOG="${LOG_PREFIX}_run${RUN}.log"
    echo ""
    echo "  --- Run $RUN/$RUNS ---"
    ./hammerdbcli auto /tmp/hammerdb_community_run.tcl 2>&1 | tee "$HAMMERDB_LOG"
...
echo ""
echo "=== MySQL Community: All $RUNS runs complete ==="
