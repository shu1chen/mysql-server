...
BASEDIR=/home/shuchen1/workspace/mysql/installation/mysql_icx_gnr_lto_hwpgo
DATADIR=/home/shuchen1/workspace/mysql/data_icx_gnr_lto_hwpgo
SOCKET=/tmp/mysql_hwpgo_test.sock
PORT=3309
HAMMERDB=/home/shuchen1/workspace/mysql/HammerDB-5.0
LOG_PREFIX=/tmp/hammerdb_hwpgo
...
echo "=== MySQL ICX/GNR/LTO/HWPGO HammerDB Benchmark ==="
...
cat > /tmp/hammerdb_hwpgo_run.tcl << 'TCLEOF'
dbset db mysql
diset connection mysql_host 127.0.0.1
...
jobs $jobid timing
TCLEOF
sed -i "s/HAMMERDB_PORT/$PORT/g; s|HAMMERDB_SOCKET|$SOCKET|g; s/HAMMERDB_RAMPUP/$RAMPUP/g; s/HAMMERDB_DURATION/$DURATION/g; s/HAMMERDB_VU_TEST/$VU_TEST/g" /tmp/hammerdb_hwpgo_run.tcl

for RUN in $(seq 1 "$RUNS"); do
    HAMMERDB_LOG="${LOG_PREFIX}_run${RUN}.log"
    echo ""
    echo "  --- Run $RUN/$RUNS ---"
    ./hammerdbcli auto /tmp/hammerdb_hwpgo_run.tcl 2>&1 | tee "$HAMMERDB_LOG"
...
echo ""
echo "=== ICX+LTO+HWPGO: All $RUNS runs complete ==="
